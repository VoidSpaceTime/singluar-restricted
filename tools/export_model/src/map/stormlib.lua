...
ffi.cdef[[
	...
	{
		char   cFileName[260];              // Name of the found file
	...
	bool SFileCreateArchive2(const char* szMpqName, struct SFILE_CREATE_MPQ* pCreateInfo, uint32_t* phMpq);
	bool SFileOpenArchive(const char* szMpqName, unsigned long dwPriority, unsigned long dwFlags, uint32_t* phMpq);
	bool SFileCloseArchive(uint32_t hMpq);
	bool SFileAddFileEx(uint32_t hMpq, const char* szFileName, const char* szArchivedName, unsigned long dwFlags, unsigned long dwCompression, unsigned long dwCompressionNext);
	bool SFileExtractFile(uint32_t hMpq, const char* szToExtract, const char* szExtracted, unsigned long dwSearchScope);
	...
	HANDLE  SFileFindFirstFile(
  		...
  		struct SFILE_FIND_DATA* lpFindFileData, // Pointer to the search result
  		const char * szListFile           // Name of additional listfile
	);

	bool  SFileFindNextFile(HANDLE hFind, struct SFILE_FIND_DATA* lpFindFileData);
	bool  SFileFindClose(HANDLE hFind);
...
function archive:seach_file(mask, filelist)
	local list = {}
	local info = ffi.new('struct SFILE_FIND_DATA')
	local handle = stormlib.SFileFindFirstFile(self.handle, mask or "*", info, filelist or "")
	if handle ~= 0 then 
		list[#list + 1] = fs.path(ffi.string(info.cFileName))
		while stormlib.SFileFindNextFile(handle, info) do 
			list[#list + 1] = fs.path(ffi.string(info.cFileName))
		end 
...
function m.open(path, readonly, filecount)
	local wpath = uni.u2a(path:string())
	local phandle = ffi.new('uint32_t[1]', 0)
	local flag = 0
	if readonly then
		flag = 0x100
